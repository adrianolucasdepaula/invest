...
- Criar arquivo específico quando mudança > 100 linhas
- Validar arquivos reais (documentação pode estar desatualizada)
- Verificar se precisa reiniciar serviços antes de testar
...
### 🎯 EXEMPLO PRÁTICO: FASE 37 (Bulk Sync & Individual Sync)

### Cenário Real: Correção de Race Condition e Limites de API

**Data:** 2025-11-23
**Fase:** FASE 37 - Bulk Sync & Individual Sync
**Complexidade:** Média (Frontend + Backend DTOs)

Este exemplo demonstra a correção de um bug crítico onde o modal de sincronização não fechava e a requisição falhava silenciosamente.
...
- Modal "Sincronizar em Massa" não fechava após clique.
- Erro `400 Bad Request` ao selecionar "Todos" (55 ativos).
- Feedback visual inconsistente (loading infinito).

**Causa Raiz:**

1.  **Frontend (Race Condition):** O modal fechava baseado apenas no HTTP 202, mas o evento WebSocket `sync:started` chegava milissegundos depois, causando conflito de estado.
2.  **Backend (Limitação):** O DTO `SyncBulkDto` tinha validação `@ArrayMaxSize(20)`, mas o portfólio tem 55 ativos.

---

### 2. Solução Definitiva

#### Frontend (`BulkSyncButton.tsx`)

Implementação de estado de espera explícito (`waitingForSyncStart`):

```typescript
// Só fecha o modal quando o WebSocket confirma o início
if (wsState.isRunning && waitingForSyncStart && !isSyncStarted) {
  setIsSyncStarted(true);
  setWaitingForSyncStart(false);
  setModalOpen(false); // ✅ Fecha modal no momento certo
  router.push("/data-management");
}
```

#### Backend (`sync-bulk.dto.ts`)

Aumento do limite para suportar todo o portfólio:

```typescript
export class SyncBulkDto {
  @ArrayMaxSize(60, { message: "Máximo 60 tickers por requisição" }) // ✅ Aumentado de 20 para 60
  tickers: string[];
}
```
...
### 3. Validação Tripla (Resultado)

1. **TypeScript Check:** 0 erros (frontend + backend).
2. **Teste Manual (Bulk Sync):** 55 ativos sincronizados com sucesso (modal fechou em 10s).
3. **Teste Regressão (Individual Sync):** Funcionalidade original preservada.
...
[
  {
    content: "1. REVISÃO FASE 35: Validar backend com testes reais",
    status: "completed",
  },
  {
    content: "2. REVISÃO FASE 35: Validar frontend com Playwright MCP",
    status: "completed",
  },
  {
    content: "3. REVISÃO FASE 35: Validar frontend com Chrome DevTools MCP",
    status: "completed",
  },
  {
    content: "4. REVISÃO FASE 35: Screenshots de evidência (3 MCPs)",
    status: "completed",
  },
  {
    content: "5. REVISÃO FASE 35: Validar TypeScript warnings (0 obrigatório)",
    status: "completed",
  },
  {
    content: "6. REVISÃO FASE 35: Verificar dados reais vs esperados",
    status: "completed",
  },
...
[
  ...
  {
    content: "2. Reiniciar backend + frontend para testes limpos",
    status: "completed",
  },
  {
    content: "3. Validação MCP #1: Playwright (UI + interação + screenshots)",
    status: "completed",
  },
  {
    content:
      "4. Validação MCP #2: Chrome DevTools (console + network + payload)",
    status: "completed",
  },
  {
    content:
      "5. Validação MCP #3: Sequential Thinking (análise profunda de lógica)",
    status: "completed",
  },
  {
    content: "6. Verificar dependências e integrações afetadas",
    status: "completed",
  },
  {
    content:
      "7. Identificar problemas crônicos (corrigir definitivo, não workaround)",
    status: "completed",
  },
...
### 6. Resultados e Métricas (Sidebar)
