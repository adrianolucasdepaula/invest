...
/**
 * MCP TRIPLO VALIDATION - FINAL VERSION
 * Robust test with proper waits for all 4 TIER 1 pages
 */

test.setTimeout(600000); // 10 minutes total

test('MCP TRIPLO - TIER 1 Complete Validation', async ({ page }) => {
  // Track results
  ...
  const ignorePatterns = [
    ...
    '403', // Ignore 403 from external widgets
  ];
  ...
  // Check stat cards (look for specific text)
  ...
  // Screenshot
  await page.screenshot({ path: 'docs/screenshots/dashboard-final-2025-12-31.png', fullPage: true });
  dashboardChecks.push('[PASS] Screenshot captured');

  const dashErrors = consoleErrors.length;
  dashboardChecks.push(`[${dashErrors === 0 ? 'PASS' : 'INFO'}] Console errors: ${dashErrors}`);
  ...
  // Wait for data to load - look for the loading indicator to disappear or data to appear
  await page.waitForSelector('text=/861 ativos|Lista de Ativos/i', { timeout: 30000 }).catch(() => null);
  await page.waitForTimeout(5000); // Extra wait for table render
  ...
  // Check search
  const hasSearch = await page.getByPlaceholder(/Buscar/i).isVisible();
  assetsChecks.push(`[${hasSearch ? 'PASS' : 'FAIL'}] Search input visible`);

  // Check asset count badge
  const hasAssetCount = await page.locator('text=/861 ativos/i').isVisible();
  assetsChecks.push(`[${hasAssetCount ? 'PASS' : 'FAIL'}] Asset count (861) visible`);

  // Check for any ticker in the table
  const hasTicker = await page.locator('text=/AALR3|ABCB4|PETR4|VALE3/i').first().isVisible().catch(() => false);
  assetsChecks.push(`[${hasTicker ? 'PASS' : 'FAIL'}] Ticker data visible`);

  // Screenshot
  await page.screenshot({ path: 'docs/screenshots/assets-final-2025-12-31.png', fullPage: true });
  assetsChecks.push('[PASS] Screenshot captured');

  const assetsErrors = consoleErrors.length;
  assetsChecks.push(`[${assetsErrors === 0 ? 'PASS' : 'INFO'}] Console errors: ${assetsErrors}`);

  const assetsPassed = assetsTitle && hasSearch && hasAssetCount;
  ...
  detailChecks.push(`[${hasIndicators ? 'PASS' : 'FAIL'}] Technical indicators section visible`);

  // Check chart canvas
  const hasChart = await page.locator('canvas').first().isVisible().catch(() => false);
  detailChecks.push(`[${hasChart ? 'PASS' : 'FAIL'}] Chart canvas visible`);

  // Screenshot
  await page.screenshot({ path: 'docs/screenshots/asset-petr4-final-2025-12-31.png', fullPage: true });
  detailChecks.push('[PASS] Screenshot captured');

  const detailErrors = consoleErrors.length;
  detailChecks.push(`[${detailErrors === 0 ? 'PASS' : 'INFO'}] Console errors: ${detailErrors}`);
  ...
  // Check for action buttons (either state - empty or with data)
  ...
  // Check for either "Criar Portfolio" or "Adicionar Posicao" depending on state
  ...
  // Check for portfolio content (either empty state message or value cards)
  const hasContent = await page.locator('text=/Valor Total|Nenhum portfólio|Nenhuma posição/i').first().isVisible();
  portfolioChecks.push(`[${hasContent ? 'PASS' : 'FAIL'}] Portfolio content visible`);

  // Screenshot
  await page.screenshot({ path: 'docs/screenshots/portfolio-final-2025-12-31.png', fullPage: true });
  portfolioChecks.push('[PASS] Screenshot captured');

  const portErrors = consoleErrors.length;
  portfolioChecks.push(`[${portErrors === 0 ? 'PASS' : 'INFO'}] Console errors: ${portErrors}`);

  const portPassed = portTitle && hasImport && hasCreateOrAdd;
