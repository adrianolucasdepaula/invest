...
export class BacktestService {
  ...
  private mapToResultDto(backtest: BacktestResult): BacktestResultDto {
    ...
    return {
      ...
      finalCapital: Number(backtest.finalCapital),
      totalReturn: Number(backtest.totalReturn),
      totalReturnPercent: Number(backtest.totalReturnPercent),
      config: backtest.config,
      riskMetrics: {
        cagr: Number(backtest.cagr),
        sharpeRatio: Number(backtest.sharpeRatio),
        sortinoRatio: Number(backtest.sortinoRatio),
        ...
        calmarRatio: backtest.calmarRatio ? Number(backtest.calmarRatio) : undefined,
      },
      tradeStats: {
        ...
        averageProfit: this.calculateAverageProfit(backtest.simulatedTrades),
        averageLoss: this.calculateAverageLoss(backtest.simulatedTrades),
        maxProfit: this.calculateMaxProfit(backtest.simulatedTrades),
        maxLoss: this.calculateMaxLoss(backtest.simulatedTrades),
      },
      incomeBreakdown: {
        premiumIncome: Number(backtest.premiumIncome),
        dividendIncome: Number(backtest.dividendIncome),
        lendingIncome: Number(backtest.lendingIncome),
        selicIncome: Number(backtest.selicIncome),
        total: totalIncome,
        premiumPercent: totalIncome > 0 ? (Number(backtest.premiumIncome) / totalIncome) * 100 : 0,
        dividendPercent: totalIncome > 0 ? (Number(backtest.dividendIncome) / totalIncome) * 100 : 0,
        lendingPercent: totalIncome > 0 ? (Number(backtest.lendingIncome) / totalIncome) * 100 : 0,
        selicPercent: totalIncome > 0 ? (Number(backtest.selicIncome) / totalIncome) * 100 : 0,
      },
      equityCurve: backtest.equityCurve,
      simulatedTrades: backtest.simulatedTrades,
      executionTime: backtest.executionTime ? Number(backtest.executionTime) : undefined,
